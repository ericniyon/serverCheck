...
class ItController extends Controller
{
    ...
    {
        $arr['users'] = User::all();
        $id=$request->verfication_id;
        $status=1;
        if($request->verfication1 == $request->verfication2){
            // DB::table('users')->where('id',$id)->update(['status'=>$status]);
            User::find($id)->update(['status'=> $status]);
            Toastr::success('Code verfied successfully :)','Success');
            return redirect('https//:www.google.com');
        }else{
            Toastr::error(' Sorry something went wrong successfully :)','Sorry');
